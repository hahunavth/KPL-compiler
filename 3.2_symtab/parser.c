...
void compileBlock(void)
{
  ...
  if (lookAhead->tokenType == KW_CONST)
  {
    ...
    do
    {
      ...
      // compileConstant();
      //
      ConstantValue *cv = compileConstant();
      obj->constAttrs->value = cv;
      // printObject(obj, 0);
      declareObject(obj);
      // symtab->program->progAttrs->scope->objList = obj;
      //
      eat(SB_SEMICOLON);
...
void compileFuncDecl(void)
{
  ...
  // // printf("%s\n", obj->name);
  // obj->funcAttrs->paramList = compileParams();
  compileParams();
  eat(SB_COLON);
  // FIXED: ADD
...
ConstantValue *compileUnsignedConstant(void)
{
  ...
  case TK_CHAR:
    eat(TK_CHAR);
    constValue = duplicateConstantValue(currentToken->value);
    break;
  default:
...
ConstantValue *compileConstant(void)
{
  ...
  case SB_PLUS:
    ...
    // constValue->intValue = compileConstant2();
    break;
  case SB_MINUS:
    eat(SB_MINUS);
    // compileConstant2();
  ...
  case TK_CHAR:
    eat(TK_CHAR);
    // FIXME: WARNING POINTER TO CHAR
    constValue = makeCharConstant((char)&lookAhead->string);
    // printf("%d\n", (lookAhead->string[0]));
    break;
  default:
    constValue = compileConstant2();
...
Type *compileType(void)
{
  ...
  case KW_ARRAY:
    ...
    eat(SB_LSEL);
    eat(TK_NUMBER);
    // type->arraySize = atoi(currentToken->string);
    int arr_len = atoi(currentToken->string);
    eat(SB_RSEL);
    eat(KW_OF);
    type = makeArrayType(arr_len, compileType());
